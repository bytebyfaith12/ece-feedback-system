...
export function AnimatedHeroShowcase() {
  ...
  const cards = useMemo(
    () => [
      { emoji: "😄", title: "Great support", text: "Thank you, team!", className: "left-2 top-5 sm:left-5 sm:top-7", delay: 0.3, style: { x: cardOneX, y: cardOneY } as MotionStyle },
      { emoji: "🙂", title: "Quick pulse", text: "One tap feedback.", className: "right-2 top-24 sm:right-5 sm:top-16", delay: 0.42, style: { x: cardTwoX, y: cardTwoY } as MotionStyle },
      { emoji: "😟", title: "Needs attention", text: "Routes to the right team.", className: "left-2 bottom-24 sm:left-8 sm:bottom-24", delay: 0.54, style: { x: cardThreeX, y: cardThreeY } as MotionStyle },
      { icon: <Radio className="size-5 text-[#061522]" />, title: "Waiting for first response", text: "Dashboard starts from zero.", className: "right-2 bottom-7 sm:right-6 sm:bottom-8", delay: 0.66, style: { x: cardFourX, y: cardFourY } as MotionStyle },
    ],
    ...
    <section className="relative mx-auto w-full max-w-[680px] overflow-visible py-3">
      ...
      >
        <motion.div
          ...
          className="absolute -inset-10 rounded-[3rem] bg-[radial-gradient(circle_at_center,rgba(0,242,254,0.24),rgba(56,248,162,0.13)_42%,transparent_72%)] blur-[58px]"
        />
        <div className="relative min-h-[520px] overflow-visible rounded-[2rem] border border-cyan-300/25 bg-[#071620]/72 p-4 shadow-[0_30px_120px_rgba(0,242,254,0.18)] backdrop-blur-xl sm:min-h-[580px] sm:p-5">
          <div className="absolute inset-0 rounded-[2rem] bg-[linear-gradient(135deg,rgba(0,242,254,0.14),rgba(56,248,162,0.08)_42%,rgba(255,255,255,0.03))]" />
          <div className="absolute inset-x-7 bottom-7 top-7 rounded-[1.8rem] bg-gradient-to-br from-cyan-200/20 via-white/10 to-emerald-200/16" />
          <div className="relative h-[520px] overflow-hidden rounded-[1.65rem] border border-white/12 bg-[#020b12] shadow-[inset_0_1px_0_rgba(255,255,255,0.12),0_24px_90px_rgba(0,0,0,0.34)] sm:h-[560px]">
            <AnimatePresence mode="wait">
              <motion.img
                key={activeProfile.src}
                src={activeProfile.src}
                alt={`${activeProfile.label} feedback profile`}
                initial={reducedMotion ? { opacity: 0 } : { opacity: 0, scale: 1.04, x: 30 }}
                animate={reducedMotion ? { opacity: 1 } : { opacity: 1, scale: 1, x: 0 }}
            ...
            <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(2,11,18,0.05),rgba(2,11,18,0.18)_50%,rgba(2,11,18,0.62))]" />
